...
const RUNTIME_UNAVAILABLE: &str = "OpenVINO runtime not available";

/// OpenVINO 推理引擎 (stub 实现)
///
/// 由于 OpenVINO 运行时需要原生库支持，此实现为 feature-gated stub。
/// 当 `openvino` feature 启用但 OpenVINO 运行时不可用时，所有操作返回错误。
...
impl InferenceEngine for OpenVinoEngine {
    ...
    async fn try_fallback_to_cpu(&mut self, _config: &ModelConfig) -> Result<(), VecboostError> {
        Err(VecboostError::InferenceError(
            "OpenVINO engine does not support CPU fallback".to_string(),
        ))
    }
}
...
mod tests {
    ...
    fn test_new_returns_runtime_unavailable_error() {
        ...
        let result = OpenVinoEngine::new(&config, Precision::Fp32);
        match result {
    ...
    fn test_new_with_fp16_precision_returns_error() {
        let config = test_config();
        let result = OpenVinoEngine::new(&config, Precision::Fp16);
        assert!(result.is_err());
    }

    #[test]
    fn test_embed_returns_runtime_unavailable_error() {
        let engine = OpenVinoEngine::new_for_test(Precision::Fp32);
    ...
    fn test_embed_batch_returns_error() {
        let engine = OpenVinoEngine::new_for_test(Precision::Fp32);
        let texts: Vec<String> = vec!["text a".to_string(), "text b".to_string()];
    ...
    fn test_precision_returns_configured_value() {
        let engine = OpenVinoEngine::new_for_test(Precision::Fp16);
        assert_eq!(*engine.precision(), Precision::Fp16);
    }

    #[test]
    fn test_supports_mixed_precision_false() {
        let engine = OpenVinoEngine::new_for_test(Precision::Fp32);
    ...
    async fn test_try_fallback_to_cpu_returns_error() {
        let mut engine = OpenVinoEngine::new_for_test(Precision::Fp32);
        let config = test_config();
    ...
    #[test]
    fn test_device_switch_cpu_returns_error() {
        let config = test_config();
        let result = OpenVinoEngine::new(&config, Precision::Fp32);
        match result {
            Err(VecboostError::InferenceError(msg)) => {
    ...
    #[test]
    fn test_device_switch_gpu_returns_error() {
